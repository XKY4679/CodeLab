"""
图片格式转换工具 1.0 —— GUI 图形窗口版
支持 PNG / JPG / WebP / BMP 批量互转
"""

import tkinter as tk
from tkinter import ttk, filedialog, messagebox
import os
import threading

try:
    from PIL import Image
    HAS_PIL = True
except ImportError:
    HAS_PIL = False

WINDOW_WIDTH = 750
WINDOW_HEIGHT = 560
IMAGE_EXTS = {".png", ".jpg", ".jpeg", ".webp", ".bmp"}
TARGET_FORMATS = {
    "PNG (.png)": ".png",
    "JPG (.jpg)": ".jpg",
    "WebP (.webp)": ".webp",
    "BMP (.bmp)": ".bmp",
}


def format_size(b):
    if b < 1024:
        return f"{b} B"
    elif b < 1024 * 1024:
        return f"{b / 1024:.1f} KB"
    else:
        return f"{b / (1024 * 1024):.2f} MB"


def scan_images(folder):
    imgs = []
    for f in sorted(os.listdir(folder)):
        full = os.path.join(folder, f)
        if os.path.isfile(full) and os.path.splitext(f)[1].lower() in IMAGE_EXTS:
            imgs.append(full)
    return imgs


def convert_image(src, dst, target_ext, quality=90):
    img = Image.open(src)
    # 处理透明通道
    if target_ext in (".jpg", ".jpeg", ".bmp") and img.mode in ("RGBA", "P", "LA"):
        bg = Image.new("RGB", img.size, (255, 255, 255))
        if img.mode == "P":
            img = img.convert("RGBA")
        bg.paste(img, mask=img.split()[-1] if "A" in img.mode else None)
        img = bg
    elif img.mode not in ("RGB", "RGBA", "L"):
        img = img.convert("RGB")

    if target_ext == ".png":
        img.save(dst, "PNG", optimize=True)
    elif target_ext == ".webp":
        img.save(dst, "WEBP", quality=quality)
    elif target_ext == ".bmp":
        img.save(dst, "BMP")
    else:
        img.save(dst, "JPEG", quality=quality, optimize=True)


class FormatConverterApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("图片格式转换工具 1.0")
        self.geometry(f"{WINDOW_WIDTH}x{WINDOW_HEIGHT}")
        self.resizable(False, False)

        self.container = tk.Frame(self)
        self.container.pack(fill="both", expand=True)

        if not HAS_PIL:
            self._show_dep_error()
        else:
            self._show_start_page()

    def _clear(self):
        for w in self.container.winfo_children():
            w.destroy()

    def _show_dep_error(self):
        self._clear()
        f = tk.Frame(self.container)
        f.place(relx=0.5, rely=0.5, anchor="center")
        tk.Label(f, text="缺少依赖库", font=("Microsoft YaHei", 20, "bold"), fg="#c62828").pack(pady=(0, 12))
        tk.Label(f, text="请在命令行中运行：", font=("Microsoft YaHei", 11)).pack()
        tk.Label(f, text="pip install Pillow", font=("Consolas", 14, "bold"), fg="#1a73e8").pack(pady=12)

    # ── 开始页面 ─────────────────────────────────────

    def _show_start_page(self):
        self._clear()
        f = tk.Frame(self.container)
        f.place(relx=0.5, rely=0.5, anchor="center")

        tk.Label(f, text="图片格式转换工具", font=("Microsoft YaHei", 24, "bold")).pack(pady=(0, 6))
        tk.Label(f, text="PNG / JPG / WebP / BMP 批量互转",
                 font=("Microsoft YaHei", 10), fg="#666").pack(pady=(0, 30))

        tk.Button(f, text="选择图片文件夹", font=("Microsoft YaHei", 13),
                  width=20, height=2, command=self._on_select).pack(pady=6)

        tk.Label(f, text="转换后保存到\"转换输出\"子文件夹，不覆盖原文件",
                 font=("Microsoft YaHei", 9), fg="#999").pack(pady=(10, 0))

    def _on_select(self):
        folder = filedialog.askdirectory(title="选择图片文件夹")
        if not folder:
            return
        imgs = scan_images(folder)
        if not imgs:
            messagebox.showwarning("提示", "文件夹中没有找到图片文件。")
            return
        self._folder = folder
        self._images = imgs
        self._show_settings()

    # ── 设置页面 ─────────────────────────────────────

    def _show_settings(self):
        self._clear()
        num = len(self._images)

        top = tk.Frame(self.container)
        top.pack(fill="x", padx=20, pady=(15, 5))
        tk.Button(top, text="← 返回", font=("Microsoft YaHei", 10), command=self._show_start_page).pack(side="left")
        tk.Label(top, text="转换设置", font=("Microsoft YaHei", 14, "bold")).pack(side="left", padx=15)

        body = tk.Frame(self.container)
        body.pack(fill="both", expand=True, padx=25, pady=10)

        tk.Label(body, text=f"已选择：{self._folder}", font=("Microsoft YaHei", 10), fg="#555").pack(anchor="w")
        tk.Label(body, text=f"共 {num} 张图片", font=("Microsoft YaHei", 11, "bold")).pack(anchor="w", pady=(2, 12))

        lb = tk.Listbox(body, height=min(5, num), font=("Microsoft YaHei", 9))
        for v in self._images:
            ext = os.path.splitext(v)[1].upper()
            lb.insert("end", f"  {os.path.basename(v)}    ({format_size(os.path.getsize(v))})")
        lb.pack(fill="x", pady=(0, 12))

        # 目标格式
        tk.Label(body, text="转换为：", font=("Microsoft YaHei", 11, "bold")).pack(anchor="w", pady=(5, 5))

        self._fmt_var = tk.StringVar(value="JPG (.jpg)")
        fmt_frame = tk.Frame(body)
        fmt_frame.pack(fill="x", pady=(0, 10))
        for label in TARGET_FORMATS:
            tk.Radiobutton(fmt_frame, text=label, variable=self._fmt_var, value=label,
                           font=("Microsoft YaHei", 11)).pack(side="left", padx=10)

        # JPG/WebP 质量
        q_frame = tk.Frame(body)
        q_frame.pack(fill="x", pady=(5, 0))
        tk.Label(q_frame, text="JPG/WebP 输出质量：", font=("Microsoft YaHei", 10)).pack(side="left")
        self._quality_var = tk.IntVar(value=90)
        tk.Scale(q_frame, from_=10, to=100, orient="horizontal", variable=self._quality_var,
                 length=300).pack(side="left", padx=10)
        tk.Label(q_frame, text="（对 PNG/BMP 无效）", font=("Microsoft YaHei", 9), fg="#888").pack(side="left")

        tk.Button(body, text="开始转换", font=("Microsoft YaHei", 13, "bold"),
                  width=16, height=2, command=self._start).pack(pady=(20, 0))

    # ── 执行 ─────────────────────────────────────────

    def _start(self):
        fmt_label = self._fmt_var.get()
        self._target_ext = TARGET_FORMATS[fmt_label]
        self._quality = self._quality_var.get()

        self._out_folder = os.path.join(self._folder, "转换输出")
        os.makedirs(self._out_folder, exist_ok=True)

        self._show_progress()
        threading.Thread(target=self._worker, daemon=True).start()

    def _show_progress(self):
        self._clear()

        top = tk.Frame(self.container)
        top.pack(fill="x", padx=20, pady=(15, 5))
        tk.Label(top, text="正在转换...", font=("Microsoft YaHei", 14, "bold")).pack(side="left")

        body = tk.Frame(self.container)
        body.pack(fill="both", expand=True, padx=20, pady=10)

        self._prog_label = tk.Label(body, text="准备中...", font=("Microsoft YaHei", 11))
        self._prog_label.pack(anchor="w", pady=(0, 3))
        self._prog_bar = ttk.Progressbar(body, length=WINDOW_WIDTH - 50, maximum=len(self._images))
        self._prog_bar.pack(fill="x", pady=(0, 10))
        self._cur_label = tk.Label(body, text="", font=("Microsoft YaHei", 10), fg="#555")
        self._cur_label.pack(anchor="w", pady=(0, 8))

        cols = ("name", "orig_fmt", "new_fmt", "size", "status")
        self._tree = ttk.Treeview(body, columns=cols, show="headings", height=14)
        self._tree.heading("name", text="原文件名")
        self._tree.heading("orig_fmt", text="原格式")
        self._tree.heading("new_fmt", text="新格式")
        self._tree.heading("size", text="新文件大小")
        self._tree.heading("status", text="状态")
        self._tree.column("name", width=260)
        self._tree.column("orig_fmt", width=80, anchor="center")
        self._tree.column("new_fmt", width=80, anchor="center")
        self._tree.column("size", width=110, anchor="center")
        self._tree.column("status", width=100, anchor="center")
        sb = ttk.Scrollbar(body, orient="vertical", command=self._tree.yview)
        self._tree.configure(yscrollcommand=sb.set)
        self._tree.pack(side="left", fill="both", expand=True)
        sb.pack(side="right", fill="y")

        self._iids = {}
        for v in self._images:
            name = os.path.basename(v)
            ext = os.path.splitext(v)[1].upper()
            iid = self._tree.insert("", "end", values=(name, ext, self._target_ext.upper(), "—", "等待"))
            self._iids[v] = iid

    def _update_row(self, path, size_str, status):
        iid = self._iids.get(path)
        if iid:
            cur = self._tree.item(iid, "values")
            self._tree.item(iid, values=(cur[0], cur[1], cur[2], size_str, status))

    def _worker(self):
        results = []
        total = len(self._images)

        for idx, src in enumerate(self._images):
            name = os.path.basename(src)
            stem = os.path.splitext(name)[0]
            dst = os.path.join(self._out_folder, stem + self._target_ext)

            self.after(0, lambda i=idx, n=name: (
                self._prog_label.config(text=f"进度：{i+1} / {total}"),
                self._prog_bar.config(value=i),
                self._cur_label.config(text=f"正在处理：{n}"),
            ))
            self.after(0, lambda p=src: self._update_row(p, "...", "转换中"))

            try:
                convert_image(src, dst, self._target_ext, self._quality)
                ns = os.path.getsize(dst)
                self.after(0, lambda p=src, s=ns: self._update_row(p, format_size(s), "完成"))
                results.append((name, True))
            except Exception as e:
                if os.path.exists(dst):
                    try:
                        os.remove(dst)
                    except OSError:
                        pass
                self.after(0, lambda p=src: self._update_row(p, "—", "失败"))
                results.append((name, False))

        self.after(0, lambda: self._done(results))

    def _done(self, results):
        self._prog_bar["value"] = self._prog_bar["maximum"]
        ok = sum(1 for r in results if r[1])
        fail = sum(1 for r in results if not r[1])
        self._prog_label.config(text=f"全部完成 ({len(results)} 张)")
        self._cur_label.config(text="")

        bar = tk.Frame(self.container, bg="#f0f0f0", relief="groove", bd=1)
        bar.pack(fill="x", padx=15, pady=(0, 12))

        left = tk.Frame(bar, bg="#f0f0f0")
        left.pack(side="left", padx=15, pady=10)
        st = f"成功 {ok} 张"
        if fail:
            st += f"，失败 {fail} 张"
        tk.Label(left, text=st, font=("Microsoft YaHei", 10, "bold"), bg="#f0f0f0").pack(anchor="w")

        right = tk.Frame(bar, bg="#f0f0f0")
        right.pack(side="right", padx=15, pady=10)
        tk.Button(right, text="打开输出文件夹", font=("Microsoft YaHei", 10),
                  command=lambda: os.startfile(self._out_folder)).pack(side="left", padx=4)
        tk.Button(right, text="返回首页", font=("Microsoft YaHei", 10),
                  command=self._show_start_page).pack(side="left", padx=4)


if __name__ == "__main__":
    app = FormatConverterApp()
    app.mainloop()
